...
![[Transformer architecture and training objectives used in GPT.png|156]] ![[Input transformations for fine-tuning on different tasks.png|600]]
...
蕴含问题：
- 蕴含问题其实就是[[Implication.pdf|Implication]]，相当于让模型去验证文本的逻辑关系。
...
如何实现不需要微调的模型呢？那就是取消所有[[GPT-123#^8a6202|特定于任务的输入转换]]，直接将任务表述成语言。
比如，如果想要将一段英语翻译成法语，那么就直接输入："翻译这段话+英文"，最后的Label是法文，最后这个方法就叫做提示——[[Prompting]]
